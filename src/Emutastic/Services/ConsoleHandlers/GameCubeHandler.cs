...
namespace Emutastic.Services.ConsoleHandlers
{
    ...
    public class GameCubeHandler : ConsoleHandlerBase
    {
        ...
        // <saveDir>/User (Saves/GameCube/User here) and checks
        // User/GC/<REGION>/IPL.bin BEFORE the bundled dolphin-emu/Sys/GC/<REGION>/
        // IPL.bin under the system dir above, so mirroring the dumps the user
        // placed in the System folder (Preferences → System Files) into the save
        // tree makes the core pick them up without touching the dolphin-emu/Sys
        // tree (which Sys/core updates may replace). With a dump present Dolphin
        // loads the official IPL fonts (fixes missing text in e.g. Star Fox
        // Assault) even while dolphin_skip_gc_bios is enabled; disabling that
        // option boots the real BIOS menu when the game-region dump exists (the
        // core gracefully re-enables skip when it doesn't). Copies are refreshed
        // when the source changes and never deleted — a user-managed file in the
        // save tree stays.
